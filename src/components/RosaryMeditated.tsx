...
export default function RosaryMeditated({
  onClose,
  onSwitchToImmersive,
  registerBackHandler,
}: MeditatedRosaryProps) {
  ...
  const getMysteryImage = useCallback((type: MysteryType, index: number) => {
    const typeShort = type.endsWith('s') ? type.slice(0, -1) : type;
  ...
  useEffect(() => {
    ...
    registerBackHandler(() => {
      if (selectedMeditationIndex === null) return false;
      setSelectedMeditationIndex(null);
      return true;
    });

    return () => registerBackHandler(null);
  }, [registerBackHandler, selectedMeditationIndex]);
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {selectedMeditationIndex === null ? (
          ...
          <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300 pb-8">
            <div
              className="w-full h-48 sm:h-64 rounded-xl bg-cover bg-top mb-6 shadow-md"
