...
export default function PlanDeVidaCalendar() {
  ...
  const allRows = useMemo(() => collectPlanDeVidaPrayers(allPrayers), [allPrayers]);
  ...
  const rows = useMemo(() => {
    if (isDeveloperMode && isEditMode) return allRows;
    return allRows.filter((row) => visibleRowIds.has(row.id));
  }, [allRows, isDeveloperMode, isEditMode, visibleRowIds]);
